...
>>>>>>> d051411 (build: clean up documentation build chain and fix broken links)
...
- **`supportedInterfaces`** (optional, array of [`AgentInterface`](#agentinterface)): An ordered list of supported interfaces (protocol binding and URL combinations). The first item in the list is the preferred interface that clients should use when possible. Clients can select any interface from this list based on their preferences, but SHOULD prefer earlier entries when multiple options are supported.
- **`provider`** (optional, [`AgentProvider`](#agentprovider)): The service provider of the agent.
...
- **`capabilities`** (required, [`AgentCapabilities`](#agentcapabilities)): A2A capability set supported by the agent.
- **`securitySchemes`** (optional, map of string to [`SecurityScheme`](#securityscheme)): The security scheme details used for authenticating with this agent.
...
- **`skills`** (required, array of [`AgentSkill`](#agentskill)): Skills represent units of ability an agent can perform.
...
- **`signatures`** (optional, array of [`AgentCardSignature`](#agentcardsignature)): JSON Web Signatures computed for this AgentCard.
...
=======
#### 4.5.10. Security {: #security }

{{ proto_to_table("specification/a2a.proto", "Security") }}

#### 4.5.11. StringList {: #stringlist }

{{ proto_to_table("specification/a2a.proto", "StringList") }}

#### 4.5.10. ImplicitOAuthFlow {: #implicitoauthflow }

{{ proto_to_table("specification/a2a.proto", "ImplicitOAuthFlow") }}

#### 4.5.11. PasswordOAuthFlow {: #passwordoauthflow }

{{ proto_to_table("specification/a2a.proto", "PasswordOAuthFlow") }}
>>>>>>> d051411 (build: clean up documentation build chain and fix broken links)
